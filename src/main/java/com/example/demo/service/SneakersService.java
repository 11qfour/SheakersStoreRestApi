...
import com.example.demo.controller.SneakersController;
...
import com.example.demo.repository.SneakersSuppliersRepository;
...
public class SneakersService {
    ...
    private final SneakersSuppliersRepository sneakersSuppliersRepository;
    ...
    public SneakersDtoResponse create(SneakersDtoRequest sneakersDto){   //метод создания кроссовок
        ...
        if (sneakersDto.getSneakerModel() == null || sneakersDto.getSneakersTypeId() == null) {
            throw new IllegalArgumentException("Необходимо указать модель кроссовок и тип");
        }
        //установка категории
    ...
    @Transactional
    public void addSneakerAndSupplierRelation(Long sneakersId, Long supplierId) {
        // Проверяем существование кроссовок
        Sneakers sneakers = sneakersRepository.findById(sneakersId).orElseGet(() -> {
            Sneakers newSneakers = new Sneakers();
            newSneakers.setId(sneakersId);
            newSneakers.setSneakerModel("Default Model"); // Указываем дефолтную модель
            return sneakersRepository.save(newSneakers);
        });

        // Проверяем существование поставщика
        Supplier supplier = supplierRepository.findById(supplierId).orElseGet(() -> {
            Supplier newSupplier = new Supplier();
            newSupplier.setId(supplierId);
            // Установите дополнительные параметры, если нужно
            return supplierRepository.save(newSupplier);
        });

        // Добавляем связь в таблицу sneakers_suppliers
        sneakersSuppliersRepository.addSneakerSupplierRelation(sneakers.getId(), supplier.getId());
    }
}
