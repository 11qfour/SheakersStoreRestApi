...
public interface SneakersMapper {
    Sneakers toEntity(SneakersDtoRequest productDto);

    @Mapping(target = "supplierIds", expression = "java(suppliersToSupplierIds(product.getSuppliers()))")  // используется для определения специфичных правил сопоставления между полями источника и назначения
    @Mapping(target = "sneakersTypeId", source = "sneakersType.id")
    SneakersDtoResponse toDto(Sneakers product); //(аннотации) извлечение идентификаторов поставщиков и идентификатора типа кроссовок
    ...
    Sneakers partialUpdate(SneakersDtoRequest productDto, @MappingTarget Sneakers sneakers); //параметр метода будет использоваться как целевой объект для маппинга
